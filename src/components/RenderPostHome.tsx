                    ...
                    post.map((postData: any) => (
                        <div className="flex flex-row border-b-2 bg-slate-200">
                            ...
                            <div className="text-left">
                                <Link to={`/post/${postData.id}`} key={postData.id} className="sm:text-xl md:text-2xl hover:underline hover:cursor-pointer ">{postData.title} <span className="text-sm text-neutral-800 hidden md:block">click to read more</span></Link>
                                <p className="sm:text-md md:text-lg sm:mr-0 md:mr-20 font-medium sm: line-clamp-3 md:line-clamp-5">{postData.description}</p>
