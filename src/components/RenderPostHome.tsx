        ...
        <div className="flex flex-row">
            <div>
                {query.isLoading ? <p className='text-2xl'>Loading ...</p> : query.isError ? <p className='text-2xl'>Error! </p> : query.data ? query.data.map((postData: Post) => (<div className="flex flex-row border-b-2 bg-slate-200 ">
                    <img className="h-40 w-60 sm:h-48 sm:w-72  sm:ml-0 md:ml-20  mr-5 border-2 border-emerald-500 mb-4" src={postData.image} alt='Post home' />
                    <div className="text-left">
                        ...
                        <div className='text-right text-lg hidden md:block sm:mr-0 md:mr-20'>
                            <Link to={postData.category} className='hover:underline hover:cursor-pointer'>{postData.category}</Link>
                        </div>
                    </div>
                </div>)) : null
                }
