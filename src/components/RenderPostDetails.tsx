        ...
        <div className='bg-slate-200 '>
            {query.isLoading ? <p className='text-2xl'>Loading ...</p> : query.isError ? <p className='text-2xl'>Error! </p> : query.data ? (
                <div className='flex flex-col sm:mx-0 md:mx-20'>
                    <img className='object-scale-down h-80 w-auto bg-slate-200 mb-5' src={query.data?.image} alt='Post main' />
                    <div>
                        ...
                        <p className='text-lg text-justify font-medium tracking-widest mb-3'>{query.data?.description}</p>
                    </div>
                </div>)
                : null
            }
